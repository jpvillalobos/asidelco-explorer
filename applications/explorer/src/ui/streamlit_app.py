...
def _validate_step_args(workspace: str, step_name: str, args: dict) -> Optional[str]:
    """Validate step arguments before execution"""
    ws = Path(workspace)
    ...
    if step_name == 'load_opensearch':
        host = args.get('host', 'localhost')
        port = args.get('port', 9200)
        index_name = args.get('index_name')
        
        if not index_name:
            return "Index name is required for OpenSearch loading"
        ...
        logger.debug(f"OpenSearch validation passed: {host}:{port}/{index_name}")
...
def _execute_steps(stage_idx: int, stage_data: dict, step_indices: Optional[List[int]] = None) -> bool:
    ...
    try:
        ...
        step_type_map = {
            ...
            "load_excel_template": "load_excel_template",  # ADD THIS
...
def _is_path_argument(arg_name: str, arg_value: str) -> tuple[bool, str]:
    ...
    if 'template' in arg_lower and 'file' in arg_lower:
            ...
            if isinstance(arg_value, dict):
                ...
                if is_path and workspace_path and workspace_path.exists():
                    ...
                    if selection_mode == "Browse workspace":
                        if path_type == 'template_file':
                            # Show template file browser
                            file_types = ['.xlsx', '.xltx', '.xls']
                            files, _ = _get_workspace_files_and_dirs(workspace_path, "*", file_types)

                            if files:
                                st.caption(f"💡 Select Excel template from workspace")
                                ...
                                st.warning(f"⚠️ No Excel template files found in workspace")
