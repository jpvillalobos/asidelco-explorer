...
class StepRegistry:
    ...
    def _load_excel_template(self, **kwargs) -> Dict[str, Any]:
        """Load JSON data to Excel template"""
        file_handler = self._setup_step_logging('excel_template', 'load_excel_template')
        logger = logging.getLogger('services.excel_template_service')
        
        if file_handler:
            logger.addHandler(file_handler)
        
        try:
            excel_service = self._get_service('excel_template')
            ...
            logger.info(f"Starting Excel template loading")
            logger.info(f"  Input: {input_file}")
            logger.info(f"  Template: {template_file}")
            logger.info(f"  Output: {output_file}")
            
            result = excel_service.load_to_template(
                input_file=input_file,
            ...
            logger.info(f"Excel loading completed: {result.get('count', 0)} rows written")
