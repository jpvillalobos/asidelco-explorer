    ...
    def run(self):
        ...
        for i, (step_type, kwargs) in enumerate(self.steps, 1):
            ...
            for key, value in kwargs.items():
                # FIXED: Check if value is a URL first, before treating as path
                if isinstance(value, str):
                    # Skip URL resolution - keep URLs as-is
                    if value.startswith(('http://', 'https://', 'ftp://', 'ws://', 'wss://')):
                        resolved_kwargs[key] = value
                        logger.debug(f"Kept URL as-is: {key}={value}")
                    # Only resolve filesystem paths (containing file/path/dir keywords, but NOT url)
                    # Exclude keys containing 'url' to avoid treating URLs as paths
                    elif ('file' in key.lower() or 'path' in key.lower() or 'dir' in key.lower()) and 'url' not in key.lower():
                        ...
                        # Not a path/file/dir keyword, or contains 'url', keep as-is
                        resolved_kwargs[key] = value
                else:
                    resolved_kwargs[key] = value
